...
| Upgraded `json-graphql-server` 3.1.1 → 3.3.1 | The bundled GraphiQL page never loads on 3.1.1
...
**Why** There's already a lot of details in the fleet and satellite detail pages. and the main goal of those detail pages is to make contact with the satellite fleet and ground control. I did allow users to comment on reports on a detail page so ongoing comms can be had. from the page

### CI: Github actions (`test`, `lint`, `lint:styles`, `format:check`, `build`)
**Why** These actions run on every surface that  push and PR, ensuring code quality and consistency.
...
**Why:** Contracts and reports come in from a non polling query and merged in cache


## Concerns with larger datasets
1. findContactWindows walks a 24-hour horizon in 30-second steps, ~2,880 SGP4 calculcations per satellite–station pair, on the main thread. On a larger fleet, I would consider moving scans to a background thread or lazy computing each row (or both)
